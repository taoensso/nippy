...
(defmacro write-typed+meta [x bb dout_]
  `(write-typed+meta* ~x ~bb ~dout_ taoensso.nippy/*incl-metadata?*))
...
(defmacro write-el
  ...
  `with-els`, whose context it uses.

  Adds an `instanceof` fast path for the hottest types. Worth it because
  a collection's elements often alternate in type, and Clojure's protocol
  dispatch caches only ONE class per call site (`MethodImplCache`) - so
  each element would otherwise miss the cache and pay a map lookup.

  No fast-path type implements `IObj` (none can carry metadata), so the
  fast path can also skip the metadata check. NB this must hold for any
  type added to the chain. Ref. `fast-writers?` re: `extend-freeze`."
  [x bb dout_]
  `(let [x# ~x]
     (if ~'-el-fast?
       (cond
         (instance? clojure.lang.Keyword x#) (write-kw*    ~bb x# ~'-el-state)
         (instance? String               x#) (write-str    ~bb x#)
         (instance? Long                 x#) (write-long   ~bb x#)
         (instance? Double               x#) (write-double ~bb x#)
...
(defmacro write-dyn-array-lg
  ...
  `(do
     ...
     (with-els
       (enc/reduce-n (fn [_# idx#] (write-el (aget ~arr idx#) ~bb ~dout_)) nil ~alen))))
...
(defn write-vec [^ByteBuffer bb dout_ v]
  (let [cnt (count v)]
    ...
    (when-not (zero? cnt)
      (with-els
        (run! (fn [el] (write-el el bb dout_)) v)))))
...
(defn write-counted-coll
  ([^ByteBuffer bb dout_ id-lg coll]
   (let [cnt (count coll)]
     ...
     (write-lg-count bb cnt)
     (with-els
       (reduce (fn [_ in] (write-el in bb dout_)) nil coll))))
...
(defn write-uncounted-coll
  ...
  ([^ByteBuffer bb dout_                      id-lg coll]
   ...
   (let [cnt-idx   (.position bb)
         ...
         ^long cnt (with-els
                     (reduce (fn [^long cnt in] (write-el in bb dout_) (unchecked-inc cnt)) 0 coll))]
...
(defn write-set
  ...
  (let [cnt (count s)]
    ...
    (when-not (zero? cnt)
      (with-els
        (reduce (fn [_ in] (write-el in bb dout_)) nil s)))))
...
(defn write-cached-header!
  ...
  (if (and (keyword? x-val) (.-auto-kws? state))
...
(defn write-kw*
  "Writes given keyword, auto-caching it when enabled for the given
  (current) session. Takes `state` so that callers writing many keywords
  can read `tl:cache` once rather than once per keyword."
  [^ByteBuffer bb ^clojure.lang.Keyword kw ^CacheState state]
  (if (and state (.-auto-kws? state)) ; Flag captured at session start
...
(let [samples [:kw "" (long 0) (double 0)] ; One per `write-el` fast-path type
  ...
  (defn fast-writers?
    "Returns true iff `write-el`'s `instanceof` fast paths are still safe to
    use, i.e. iff no `extend-freeze` has replaced Nippy's own writer for any
    of the fast-path types (directly, or via an interface/ancestor).

    Cheap: `extend` always installs a NEW protocol map, so the usual case
    needs only an identity check. Called once per collection."
    []
