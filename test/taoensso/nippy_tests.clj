...
(deftest _types-fast-path
  ...
  (let [protos
        [#'io/IWriteTypedNoMeta #'io/IWriteTypedNoMetaDin
    ...
    (try
      [(is (io/fast-writers?) "Fast path enabled by default")
       (is (not-any? #(instance? clojure.lang.IObj %)
             (map second @#'io/fast-path-writers))
         "No fast-path type is `IObj` (fast path skips the metadata check)")
...
(def ref-hashes
  (merge
    {:lazy-seq-empty -574080456, :true -1809580601, :long 219451189, :double -454270428,
     :lazy-seq -856460618, :short 1152993378, :meta 352218350, :str-long -1970041891,
     :instant -1401948864, :many-keywords 165443084, :bigint 2033662230, :sym-ns 769802402,
     :queue 447747779, :float 603100813, :sorted-set 1443292905, :many-strings 1777678883,
     :nested 569092370, :queue-empty 1760934486, :duration -775528642, :false 1506926383,
     :vector -2129355781, :util-date 1326218051, :kw 389651898, :sym -1742024487,
     :str-short -1097575232, :subvec -2047667173, :kw-long 852232872, :integer 624865727,
     :sym-long -1535730190, :list -1113199651, :ratio 1186850097, :byte -1041979678,
     :bigdec -1846988137, :nil 2005042235, :defrecord 410230155, :sorted-map 1464032648,
     :sql-date 80018667, :map-entry -1353323498, :false-boxed 1506926383, :uri -1374752165,
     :period -2043530540, :many-longs 759118414, :uuid -338331115, :set -1515144175,
     :kw-ns 1050084331, :map 358912619, :many-doubles -827569787, :char 858269588}

    ;; Unsigned packing (`impl/pack-unsigned?`) writes ids added in Nippy v3.3.0
    (when (impl/target-release< 330)
      {:defrecord 842721251, :list -1207486853, :long 598276629, :many-longs -1109794519,
       :many-strings 1738215727, :map 1989337680, :map-entry 1219306839, :meta -858252893,
       :nested -50526117, :set 1649942133, :sorted-map -1160380145, :sorted-set 2005004017,
       :str-short -921330463, :subvec 709331681, :uri 870148616, :vector 813550992})

    {:deftype
     (enc/cond
       (impl/target-release>= 370) -1858572854
       (impl/target-release>= 330)  -602098807
       :else                       -671450876)}))
